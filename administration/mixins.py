...
class ManagerCommentAndLoginRequiredMixin(AccessMixin):
    def dispatch(self, request, *args, **kwargs):
        try:
            user = self.request.user
            id = self.request.session['project_id']
            ...
            return redirect("dashboard:dashboard-chart")
        if not request.user.is_authenticated or not request.user.role == 'project_manager'or project.archive or not int(id) in project_id or not user.id in author:
            return redirect("dashboard:dashboard-chart")
        return super().dispatch(request, *args, **kwargs)
...
class TesterAndLoginRequiredMixin(AccessMixin):
    def dispatch(self, request, *args, **kwargs):
        try:
            ...
            ticket_status_id = Ticket.objects.filter(id=self.request.path.split('/')[4], organisation=user.member.organisation).values_list('status_id', flat=True)[0]
        except:
            return redirect("dashboard:dashboard-chart")
        if not request.user.is_authenticated or not request.user.role == 'tester'or project.archive or not int(id) in project_id or not ticket_status_id in status_id:
            return redirect("dashboard:dashboard-chart")
        return super().dispatch(request, *args, **kwargs)
...
class TesterCommentAndLoginRequiredMixin(AccessMixin):
    def dispatch(self, request, *args, **kwargs):
        try:
            ...
            project_id = Project.objects.filter(title=user.ticket_flow, organisation=user.member.organisation).values_list('id', flat=True)
            status_id = Status.objects.filter(test_status=True, organisation=user.member.organisation).values_list('id', flat=True)
            ticket_status_id = Ticket.objects.filter(id=self.get_object().ticket.id, organisation=user.member.organisation).values_list('status_id', flat=True)[0]
            author = Comment.objects.filter(id=self.kwargs["pk"]).values_list('author', flat=True)
        except:
            return redirect("dashboard:dashboard-chart")
        if not request.user.is_authenticated or not request.user.role == 'tester' or project.archive or not int(id) in project_id or not ticket_status_id in status_id or not user.id in author:
            return redirect("dashboard:dashboard-chart")
        return super().dispatch(request, *args, **kwargs)
